...
How claude-harness is published and listed. Three channels, in increasing effort.
...
| Field | Value |
|---|---|
| Display Name | `claude-harness` |
| Category | `Tooling` |
| Sub-Category | `Tooling: Config Managers` |
| Primary Link | `https://github.com/Wombat164/claude-harness` |
...
- **Install:** `/plugin marketplace add https://github.com/Wombat164/claude-harness` then
  `/plugin install claude-harness`; or `pipx install claude-harness` for the CLI.
- **Uninstall:** `/plugin uninstall claude-harness` (+ `/plugin marketplace remove claude-harness`); or
  `pipx uninstall claude-harness`.
...
- **Demo:** the docs site (https://wombat164.github.io/claude-harness/) + the runnable synthetic
  `examples/vault/`.
...
(direct PRs to that repo are auto-closed -- the form is the only path). Provide: name `claude-harness`,
repository `https://github.com/Wombat164/claude-harness` (marketplace.json at root), the description above,
category *knowledge management / tooling*, license MIT. The stricter `anthropics/claude-plugins-official`
is a later stretch goal.

## Versioning reminder

Bump `version` in `pyproject.toml`, `.claude-plugin/plugin.json`, and `claude_harness/__init__.py` together
